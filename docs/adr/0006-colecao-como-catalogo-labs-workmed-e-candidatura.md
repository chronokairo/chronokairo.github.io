...
8. **Nimbus como produto entregue.** Nimbus deixa de aparecer como "base desktop" ou "prévia" e passa a ser descrito pelo que já opera: painel DevOps (servidores, SSH, Docker/Kubernetes, monitoramento, CI/CD, deploy e backup), cinco provedores free tier e failover por cota. O visual 3D da home foi refeito como um instrumento com câmera ortográfica, em traço fino e monocromático, que segue o tema claro ou escuro. Cada provedor é uma coluna cujo preenchimento mostra a cota usada, com um colar em 85%. O tráfego percorre a rota ativa e migra sozinho para o provedor mais saudável, e um painel mostra uso e estado ao vivo. A cena é rotulada como simulação ilustrativa e respeita `prefers-reduced-motion`.
